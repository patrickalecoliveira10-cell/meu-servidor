...
class Intelligence {
  constructor() {
    this.minConfidence = 0.60; // Sniper V1.2.2
  }

  async init(brain) {
    this.brain = brain;
    logger.info('[INTEL] Sniper Ativado.');
  }
  ...
  async analyze(snapshot, weights, config) {
    try {
      ...
      const decision = confidence >= this.minConfidence ? 'enter' : 'wait';
  ...
  analyzeLivePosition(snapshot, position) {
    ...
    if (profit >= 2.0) return { action: 'partial_exit', params: { percent: 0.5 }, reason: "Parcial" };
    if (profit < -2.0) return { action: 'close', reason: "Stop Realizado" };
